                            ...
                            for iface in addrs {
                                if let get_if_addrs::IfAddr::V4(ifv4) = iface.addr {
                                    ...
                                    for i in 1..=max_hosts {
                                        ...
                                        let peer_db = peer_db.clone();
                                        let ssl = ssl.clone();
                                        tasks.push(tokio::spawn(discover_and_track_cichlid(addr, peer_db, ssl)));
                                    }
                                }
                            }
                        }
...
async fn discover_and_track_cichlid(addr: SocketAddr, peer_db: Arc<sled::Db>, ssl: Arc<SslConnector>) {
    ...
    let mut tls = match SslStream::new(ssl, tcp) {
        Ok(stream) => stream,
        Err(_) => {
            trace!("SslStream construct fail");
            return;
        }
    };
    if Pin::new(&mut tls).connect().await.is_err() {
        trace!("TLS handshake failed for {addr}");
        return;
    }
    ...
    if tls.write_all(http_req.as_bytes()).await.is_err() {
        return;
    }
    let mut buf = Vec::new();
    if tls.read_to_end(&mut buf).await.is_err() {
        return;
    }
    let response = String::from_utf8_lossy(&buf);

    // Persist PeerRecord if health is OK/parsed:
    let now = chrono::Utc::now().timestamp();
    if response.contains("200 OK") && response.contains("version") {
        let health_json = response.to_string(); // optional: parse to just the health section
